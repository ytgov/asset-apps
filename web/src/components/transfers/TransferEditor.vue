	...
	<v-navigation-drawer v-model="drawer" temporary absolute right width="600px">
		...
		<v-sheet class="mx-5 mt-5">
			...
			<v-dialog v-model="isShowingDeleteDialog" max-width="400">
				<v-card>
					<v-card-title v-if="!item.asset_item_id"
						>Remove Transfer</v-card-title
					>
					<v-card-title v-if="item.asset_item_id">Revert Transfer</v-card-title>
					<v-card-text>
						<p>
							Are you sure you want to permanently remove this transfer record?
						</p>

						<p class="text-error mb-0" v-if="item.asset_item_id">
							<em
								>* This action returns the tagged asset to its original owner
								and sets its status to active.</em
							>
						</p>
					</v-card-text>
					<v-divider></v-divider>
