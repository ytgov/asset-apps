    ...
    async (req: Request, res: Response) => {
        ...
        mailcodeData.forEach((mc: any) => {
            ...
            if (mailcode == keyword)
                ...
                results.push(Object.assign(mc, { priority: mailcode.indexOf(keyword) }));
            else if (description.indexOf(keyword) >= 0)
                results.push(Object.assign(mc, { priority: description.indexOf(keyword) }));
            else if (location.indexOf(keyword) >= 0)
                results.push(Object.assign(mc, { priority: location.indexOf(keyword) }));
        });

        results = results.sort((a, b) => { return a.priority - b.priority });
        let cleaned = results.map(r => { return { department: r.department, mailcode: r.mailcode, location: r.location, description: r.description, display_name: `${r.mailcode} : ${r.description} (${r.department}) - ${r.location}` } })
