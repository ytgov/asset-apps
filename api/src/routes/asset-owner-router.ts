...
assetOwnerRouter.get("/", async (req: Request, res: Response) => {
...
assetOwnerRouter.get(
  ...
  async (req: Request, res: Response) => {
    ...
    if (list) {
      let count = await db("asset_item")
        .where({ asset_owner_id: id, status: "Active" })
        .count("* as counter")
        .first();
      list.asset_count = (count as any).counter;

      let transfers = await db("asset_transfer")
        .where({ from_owner_id: id })
        .orWhere({ to_owner_id: id })
        .count("* as counter")
        .first();
      list.transfer_count = (transfers as any).counter;

      list.managers = new Array<any>();
      let managers = await db("user").whereRaw(
        `manage_mailcodes LIKE '%${list.mailcode}%'`
      );
...
assetOwnerRouter.put(
  ...
  async (req: Request, res: Response) => {
    ...
    return res.json({
      data: list,
      messages: [{ text: "Owner saved", variant: "success" }],
    });
  }
);
...
assetOwnerRouter.delete("/:id", async (req: Request, res: Response) => {
  let { id } = req.params;
  ...
  if (ownedAssets.length == 0) {
    ...
    return res.json({
      data: {},
      messages: [{ variant: "success", text: "Owner removed" }],
    });
  } else {
    return res.json({
      data: {},
      messages: [
        { variant: "error", text: "Owner has assets assigned, remove denied" },
      ],
    });
  }
});

function cleanAndDefault(
  input: string,
