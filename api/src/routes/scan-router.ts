    ...
    async (req: Request, res: Response) => {
        ...
        for (let item of list) {
            ...
            if (item.scan_value && item.scan_value.startsWith("Y")) {
                item.asset = await db("asset_item").where({ tag: item.scan_value }).first();

                if (item.asset) {
                    item.description = `${item.scan_value} : ${item.asset.description}`;

                    if (item.asset.make)
                        item.description += " - " + item.asset.make

                    if (item.asset.model)
                        item.description += " " + item.asset.model
                }
            }
        }
...
scanRouter.delete("/:id", [param("id").notEmpty().isInt()], ReturnValidationErrors,
    async (req: Request, res: Response) => {
        let { id } = req.params;

        await db("asset_scan").where({ id }).delete();

        return res.json({ messages: [{ variant: "success", text: "Scan deleted" }] });
    });
